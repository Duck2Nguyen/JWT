...
import { updateFaceRegistration } from '../../redux/apiRequest';
import { loginSuccess, updateCurrentUser } from '../../redux/authSlice';
...
function FaceRegistration() {
    ...
    const faceIoScriptLoaded = () => {
        ...
        if (faceIO && !faceioInstance) {
            faceioInstance = new faceIO('fioaa1c6')
        }
    }
    // Đăng ký khuôn mặt mới vào hệ thống
    // 4ae38638060d4790822ac67dc8f625a8fioaa1c6
    const faceRegistration = async () => {
        ...
        try {
            const userInfo = await faceioInstance.enroll({
                ...
                payload: {
                    data: JSON.stringify({ ...other, faceRegistration: true }),
                },
            })
            await dispatch(updateCurrentUser())
            await updateFaceRegistration(dispatch, data, accessToken, axiosJWT)
